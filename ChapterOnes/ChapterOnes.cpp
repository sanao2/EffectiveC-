...
//--------------------------------------------------------------------------------------------------------------상수 키워드 const 를 define 대신 사용해보면 좋다.
...
public:
	...
	/// 
	/// [ 즉, 템플릿으로 받는 T가 사용이 될 때 받는 실제 타입을 복사 / 생성 하면서, 같은 함수 이름이지만 다른 타입을 사용할 수 있다. ]
	/// 
	/// 인라인 함수와의 관계란? 
	...
	template <typename T>
	inline T CallWithMin(const T& a, const T& b) // 어떤 타입으로 적용할 지 헷갈릴 가능성이 있지 않을까? 
	{
...
/// 
///  [ 반환 타입에 대한 상수 멤버 함수인지, 아니면 멤버 함수 자체가 상수인지 주의 필요. ]
///  [ 상수가 선언된 멤버 함수인지, 아닌지에 따라 호출 할 수 있는 멤버 함수 타입이 달라짐. ] 
/// </summary>
...
///        안정성에는 문제 없지만, (비상수 멤버 함수 안에서는 객체를 바꾸든 안바꾸든 자유이기 때문에, 상수 멤버 함수를 호출한다고 해서 특별히 문제는 없다) 
...
/// 
/// [ 즉, 대입 과 초기화의 차이를 잘 이해하고, 멤버 초기화 리스트의 사용을 일상화 하자! ]
/// </summary>
...
/// 
///  멤버 초기화 리스트의 사용을 일상화 하는 것이 더 안정성을 높이고, 사용자의 실수를 줄일 수 있다!
...
/// 
/// [ 즉, 대입 으로 인한 값 할당 보다, 멤버 초기화 리스트를 사용해 초기화 하는 것을 일상화 하자. ]
/// </summary>
