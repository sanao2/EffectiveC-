...
/// 
///  식별자가 어디에서 선언되고 호출되는 지, 어디까지 유효한지, 이 식별자가 어떤 타입이고 어떤 속성을 갖고 있는지 등을 관리하고 
/// 
/// 기호 테이블의 목적. 
///  1. 식별자 관리. 
...
///  결과적으로 중복된 상수 값이 줄어들고, 멤모리 사용이 더 효율적이다. 
...
void cClassTest() {
	cClass c;
	c.NumScore;			// 가져와서 읽기만 가능 항목 3 참고 
	c.scores[0] = 1;	// 배열의 크기는 컴파일 타임에 결정되므로, 배열 크기에 const 키워드를 사용하면 컴파일 타임에 결정된 상수로 사용할 수 있다.
	cout << "c.scores[0] : " << c.scores[0] << endl << " scores Size : " << c.NumScore << endl; // 배열의 크기를 출력한다. 

	// 클래스 상수는 객체를 생성하지 않고도 접근할 수 있다. 
	int StaticscoreNum = c.NumScore;	  // 정적 클래스 상수  : 객체가 가진 딱 하나의 메모리만 만들어 공유 
	int StaticscoreNum2 = c.NumScore;
	int StaticscoreNum3 = c.NumScore;

	cout << "cClass 클래스 정적 상수 주소 : " << &c.NumScore << endl; // 클래스 상수의 주소를 출력한다.
	cout << "정적 클래스 상수  : 객체가 가진 딱 하나의 메모리만 만들어 공유" << endl;
	cout << endl;
	cout << "StaticscoreNum : " << StaticscoreNum << " 주소 : " << &StaticscoreNum << endl;
	cout << "StaticscoreNum2 : " << StaticscoreNum2 << " 주소 : " << &StaticscoreNum2 << endl;
	cout << "StaticscoreNum3 : " << StaticscoreNum3 << " 주소 : " << &StaticscoreNum3 << endl;
	cout << endl;

	int ScoreNum = c.NumScoreTwo;		  // 비정적 클래스 상수 : 객체마다 새로운 메모리를 할당 받는다. 
	int ScoreNum2 = c.NumScoreTwo;
	int ScoreNum3 = c.NumScoreTwo;
	// 클래스 상수의 값은 선언시 바로 초기화 되므로, 
	cout << "cClass 클래스 비정적 상수 주소 : " << &c.NumScoreTwo << endl; // 클래스 상수의 주소를 출력한다. 
	cout << "비정적 클래스 상수 : 객체마다 새로운 메모리를 할당 받는다" << endl;
	cout << endl;

	cout << "ScoreNum : " << ScoreNum << " 주소 : " << &ScoreNum << endl;
	cout << "ScoreNum2 : " << ScoreNum2 << " 주소 : " << &ScoreNum2 << endl;
	cout << "ScoreNum3 : " << ScoreNum3 << " 주소 : " << &ScoreNum3 << endl;
}
...
void cClassTestTwo()
{
	...
	std::cout << "=== 비정적(non‐static) 멤버 상수 NumScoreTwo 주소 확인 ===\n";
...
private:
	...
	/// 
	/// 2. 정수 타입의 const 객체에 대해서 저장공간을 준비하지 않겠지만, 조금 좋지 않게 만들어진 컴파일러는 반대로 동작. (정수 타입의 const 객체에 대한 메모리를 따로 준비하지 않는다. 그러나 하위 컴파일러들은 그 반대로 적용할 수 있음.) 
...
public:
	...
	///   - 표현식을 매크로로 넘길 때 골치 아픈 일이 발생. 
	///   - 매크로 비교 연산에 의한 처리한 결과가 어떤 것이냐에 따라 달라질 위험 있음. 
	/// 
	/// 즉, 함수처럼 보이는 매크로를 구현할 시, 표현식을 매크로에 넘길 때 비교 연산에 따라서 처리한 결과가 달라질 수 있다. 
	/// </summary>
	/// 
	...
	template <typename T>
	inline T CallWidthMin(const T& a, const T& b) // 어떤 타입으로 적용할 지 헷갈릴 가능성 있지 않을까?
	{
...
/// 
///  반환 타입에 대한 상수 멤버 함수인지, 아니면 멤버 함수 자체가 상수인지 주의할 필요 있음. 
///
/// </summary>
...
/// 
/// 
/// 컴파일러의 검열을 통과하려면, 비트 수준의 상수성이 지켜져야 한다! 
///  
/// 이를 해결하려면, mutable 키워드가 있다. 
...
/// 
/// 정적 객체 자체를 사용하지 않고, 그 객체에 대한 참조자를 반환하는 함수를 사용하게 함으로서 정해져 있지 않은 초기화 순서에 대한 문제를 대비할 수 있다. 
...
public:
	...
	//Player() = default; // 디폴트 생성자를 사용한다고 명시. 
	//Player() = delete; // 디폴트 생성자를 삭제했다고 명시. 
};
